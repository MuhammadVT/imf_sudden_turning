...
def plot_superposed_aualae(ax, param="au", df_turn=None, stable_interval=30, 
    ...
    color_dict = {"AU":"k","AL":"g", "AE":"orange"}
    ...
    for dtm in dtms:
        ...
        ax.plot(df.relative_time, df.loc[:, param], color=color_dict[param.upper()], linewidth=1.5)
...
if __name__ == "__main__":
    ...
    params = ["au", "al", "ae", "symh"]
    ylim_au=[-50, 500]; ylim_al=[-500, 50]
    ylim_ae = [-50, 800]; ylim_symh = [-80, 30]
    ylims = [ylim_au, ylim_al, ylim_ae, ylim_symh]
	...
	if param == "symh":
	    plot_superposed_symh(ax, df_turn=df_events,
				 stable_interval=stable_interval, IMF_turning=IMF_turning, color="b")
	else:
    ...
    fig_dir = "../plots/superposed_aualae_symh/"
    fig_name = "AUALAE_SymH_for_" + str(nevents) + "_" + IMF_turning + "_turnings_IMF" 
